...
Machine learning is equivalent to forming the relationship function: y=f(X) or Y=f(X) between X (independent variables) and y (dependent variable) or between X (independent variables) and Y (dependent variables).

y or Y is the target dependent variable(s) and X is independent variables affecting y or Y.
